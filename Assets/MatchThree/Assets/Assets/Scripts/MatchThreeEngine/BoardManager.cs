    ...
    public sealed class BoardManager : MonoBehaviour
    {
        ...
        private TileData[,] Matrix
        {
            get
            {
                ...
                Debug.Log("data" + data.Length.ToString());
                return data;
            }
        }
        ...
        private void Start()
        {
            ...
            this.point = 0;
            MatchTheeGameManager.instance.numberSelectOngame = 6;
            MatchTheeGameManager.instance.numberShuffleOngame = 6;
        ...
        private async void Select(Tile tile)
        {
            ...
            Debug.Log("selection" + _selection.Count);
            var matrix = Matrix;
        ...
        private async Task<bool> TryMatchAsync()
        {
            ...
            while (match != null)
            {
                ...
                SoundController.instance.MatchSound();
                foreach (var tile in tiles)
                {
                    this.point += tile.Type.value;
                }
                UserManager.instance.useData.gold++;
                MatchTheeGameManager.instance.InitText();
                if (this.point >= UserManager.instance.useData.highScoreMatch3)
                {
                    UserManager.instance.useData.highScoreMatch3 = this.point;
                    
                }
                UserManager.instance.SaveData();
                //Debug.Log("gold" + this.point);
        ...
        public void Shuffle()
        {
            if (MatchTheeGameManager.instance.numberShuffleOngame <= 0 && !isEndGame)
            {
                Debug.Log("GameOver");
                PopupManager.Instance.OnUI(PopupCode.WinLosePanelSweet);
                isEndGame = true;
                return;
            }
            //SoundController.instance.AudioButton();
            MatchTheeGameManager.instance.numberShuffleOngame -= 1;
            MatchTheeGameManager.instance.InitText();
            _isShuffling = true;
        ...
        {
            SoundController.instance.AudioButton();
            if (MatchTheeGameManager.instance.numberSelectOngame <= 0)
            {

                PopupManager.Instance.OnUI(PopupCode.ShopHurtPanel);
            }else
            {
                MatchTheeGameManager.instance.numberSelectOngame -= 1;
                MatchTheeGameManager.instance.InitText();
                var bestMove = TileDataMatrixUtility.FindBestMove(Matrix);
        ...
        {
            SoundController.instance.AudioButton();
            if (MatchTheeGameManager.instance.numberShuffleOngame <= 0)
            {

                PopupManager.Instance.OnUI(PopupCode.ShopShufflePanel);
            }
            else
            {
                MatchTheeGameManager.instance.numberShuffleOngame -= 1;
